...
class ReactAgent:
    def __init__(self,
        ...
        # Token-window ceiling for prompt + scratchpad. gpt-4o-mini has 128k input
        # but we cap conservatively to leave room for completion + tool noise.
    ...
    def is_halted(self) -> bool:
        if self.finished:
            return False
        if self.token_budget is not None and self.ledger.total_tokens >= self.token_budget:
            return True
    ...
    def load_tools(self, tools: List[str], planner_model_name=None) -> Dict[str, Any]:
        ...
        for tool_name in tools:
            ...
            cls = getattr(module, tool_name[0].upper() + tool_name[1:])

            if tool_name == 'planner' and planner_model_name is not None:
                tools_map[tool_name] = cls(
...
if __name__ == '__main__':
    ...
    print(json.dumps(agent.ledger.snapshot(), indent=2))
